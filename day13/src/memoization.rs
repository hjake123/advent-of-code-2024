...
*/
fn hunt_solution(machine: &Machine, claw: Point, tokens_spent: usize, a_presses: usize, b_presses: usize, cache: &mut HashMap<(usize, usize), Option<usize>>, has_max_depth: bool) -> Option<usize> {
    if has_max_depth && (a_presses > 100 || b_presses > 100) {
        return None
    }
    if cache.contains_key(&(a_presses, b_presses)) {
        return *cache.get(&(a_presses, b_presses)).unwrap();
    }
    if claw == machine.prize {
        cache.insert((a_presses, b_presses), Some(tokens_spent));
        return Some(tokens_spent)
    }

    if claw.x > machine.prize.x || claw.y > machine.prize.y {
        cache.insert((a_presses, b_presses), None);
        return None
    }
    ...
    if after_pressing_a.is_none() {
        ...
        cache.insert((a_presses, b_presses), after_pressing_b);
        return after_pressing_b
    }
    if after_pressing_b.is_none() {
        cache.insert((a_presses, b_presses), after_pressing_a);
...
fn calc_destination(a_presses: usize, b_presses: usize, machine: &Machine) -> Point {
    let x = machine.a_move.x * a_presses + machine.b_move.x * b_presses;
    let y = machine.a_move.y * a_presses + machine.b_move.y * b_presses;
    Point {x, y}
}

fn calc_cost(a_presses: usize, b_presses: usize) -> usize {
...
pub fn run_b(input: &str) -> usize {
    ...
    loop {
        ...
        machine.prize = Point{x: machine.prize.x + 10000000000000, y: machine.prize.y + 10000000000000};
    }
    sum
}
